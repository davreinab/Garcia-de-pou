...
### Handoff rama crítica — `codex/critical-change-2026-05-19`

Rama creada desde `codex/mobile-category-card-actions` para aislar cambios críticos posteriores. Estado actual sin subir/commitear salvo que se indique lo contrario.

#### Acabados / swatches en cards

- `gdpProductCard(d)` soporta ahora:
  - `finishes: ['#C0C0C0', '#8C8C8C', ...]`
  - `finishCount: number`
- Render helper: `_gdpFinishes(d, mod)`.
- Regla de negocio:
  - siempre se muestra mínimo 1 cuadrado;
  - si hay 1 acabado: 1 swatch;
  - si hay 2 acabados: 2 swatches;
  - si hay más de 2: 1 swatch + `+X`;
  - `X` se limita a máximo 24.
- Tamaños:
  - `.offer-finish-swatch`: 12×12px;
  - `.offer-finish-more`: 13px máximo.
- Desktop categoría:
  - los acabados van en `.offer-code-row`, en la misma línea que `REF`, alineados a la derecha.
- Mobile categoría:
  - `.offer-finishes--copy` se oculta;
  - `.offer-finishes--price` se muestra junto al precio mediante `.offer-price-row`.
- Home / Artículos destacados:
  - en mobile sigue el mismo patrón que PLP mobile;
  - en desktop se mantiene el patrón específico de `.offers-grid`.
- `category.html` ya incluye datos de ejemplo de acabados para los productos de PLP.

#### Favorito / corazón

- El corazón vive dentro de `.offer-card-copy`.
- En category desktop NO debe posicionarse relativo a `.offer-card-copy`, porque baja el icono. Debe seguir arriba a la derecha de la card.
- En mobile category sí se posiciona contra `.offer-card-copy` para alinearse con `REF`.
- En desktop de `.offers-grid`, el corazón sí se alinea con el `CODE` dentro de `.offer-card-copy`.

#### Ajustes tipográficos recientes

- `.offer-personalizable` usa `font-weight: 500`.
- `.btn-blue` y `.btn-add-cart` usan `font-weight: 500`.
